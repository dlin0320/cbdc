...
typealias TokenAmount = Amount<String>

// referenced from [net.corda.core.contracts.Amount]
@CordaSerializable
data class Amount<T : Any>(val quantity: Long, val displayTokenSize: BigDecimal, val token: T) : Comparable<Amount<T>> {
    ...
    companion object {
        fun fromDecimal(displayQuantity: BigDecimal, token: String = "0901"): TokenAmount =
            fromDecimal(displayQuantity, token, RoundingMode.HALF_EVEN)

        fun fromLong(quantity: Long, token: String = "0901"): TokenAmount = zero(token).copy(quantity)
        ...
        fun getDisplayTokenSize(token: Any): BigDecimal {
            ...
            if (token is String) {
                return BigDecimal.ONE.scaleByPowerOfTen(-2)
            }
